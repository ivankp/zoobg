...
with requests.Session() as s:
    ...
    while True:
        html = s.get(url).text
        if html.find('GAME COMPLETE')!=-1:
            if html.find('<TD>Congratulations!</TD>')!=-1: print 'Won'
            else: print 'Lost'
            break

        while not current(html,'30030'):
            time.sleep(20)
            html = s.get(url).text
...
''' % (g.match,g.board) )[0]
        ...
        s.post(url, move)
        if len(sys.argv)<4: time.sleep(10)
        elif sys.argv[3]=='1': break
